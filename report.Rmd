...
caption_4 <- 'Scree plot of principal component analysis variance explained. Bar heights indicate the percentage of total variance explained by each principal component (PC), and the overlaid line represents the cumulative variance explained across components. Analysis was performed using prcomp() in R.'
caption_5 <- 'Principal component analysis (PCA) biplot of PC1 and PC2. Each point represents a sample, colored by colorectal cancer subtype classification. Axis labels indicate the percentage of total variance explained by each component. Analysis was performed using prcomp() in R.'
caption_8 <- 'Heatmap of row-scaled differential probe intensities. Red cells indicate higher intensity relative to the probe mean (positive z-score), while blue cells indicate lower intensity (negative z-score). Rows represent probes and columns represent samples.'
```
...
bar_cols <-c('Variance Explained'='light blue')
  variance_tibble %>%
        mutate(principal_component = factor(principal_component, 
                                         levels = unique(principal_component[order(as.numeric(gsub("PC", "", principal_component)))]))) %>%
    ggplot() + 
    geom_bar(aes(x=principal_component, y=variance_explained, fill='Variance Explained'), stat='identity', color='black') + 
    geom_line(aes(x=principal_component, y=cumulative, group=1, color='Cumulative')) +
    geom_point(aes(x=principal_component, y=cumulative, group=1, color='Cumulative')) +
